...
char * modes[] = {"Static", "Wipe", "Rainbow", "Rainbow Cycle", "Stars", "Flicker", "Fade", "Cycle", "Chase", "Blinking", "Alternate Blinking", "Smooth Blinking", "Smooth Alternate Blinking", nullptr};
...
void modeinit(String m, int col, bool rnd) {
  if(m == "Wipe") {
    ...
    blinking_i = 0;
    blinking_p = false;
  } else if(m == "Alternate Blinking") {
    ablinking_i = 0;
    ablinking_p = 0;
  } else if(m == "Smooth Blinking") {
    sblinking_i = 256;
    sblinking_p = 4;
  } else if(m == "Smooth Alternate Blinking") {
    sablinking_i = 0;
    sablinking_p = 0;
    if (rnd) {
...
void modestep(String m, int col, bool rnd, double speed) {
  if(speed == 0)
    return;
  if(m == "Wipe") {
    if(rnd) {
...
void loop() {
  ...
  if (laston != on.getValue().boolean) {
    Serial.print("on: ");
    Serial.println(on.getValue().boolean?"true":"false");
    laston = on.getValue().boolean;
  }
  
  if (lastbrightness != brightness.getValue().integer) {
    lastbrightness = brightness.getValue().integer;
    Serial.print("brightness: ");
    Serial.println(lastbrightness);
    strip.setBrightness(lastbrightness*2.54+1);
    strip.show();
  }

  if (on.getValue().boolean) {
    if (!mode.getValue().string) return;
    String newmode = *(mode.getValue().string);
    int newcolour = str2intColour(*(colour.getValue().string));
    bool newrandom = rndm.getValue().boolean;
    double newspeed = speed.getValue().integer/100.0;
    if(newmode != lastmode || newcolour != lastcolour || newrandom != lastrandom) {
      lastmode = newmode;
      ...
      modeinit(newmode, lastcolour, lastrandom);
    }
    ...
    modestep(newmode, lastcolour, lastrandom, lastspeed);
  } else {
    lastmode = "";
    strip.fill(0x000000, 0, strip.numPixels());
    strip.show();
